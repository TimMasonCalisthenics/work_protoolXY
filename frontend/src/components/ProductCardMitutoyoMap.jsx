...
function ProductCardMitutoyoMap({ actualValue, image, name, detail, onComplete, precision = 3 }) {
  const [mitutoyo, setMitutoyo] = useState({ value: actualValue ?? '', status: '' });
  const mitutoyoConfig = { min: detail.min_value, max: detail.max_value };
  useEffect(() => {
    if (actualValue !== undefined && actualValue !== null) {
      handleMitutoyoChange(actualValue);
    }
  }, [actualValue]);
  const handleMitutoyoChange = (value) => {
    ...
    const formattedValue = isNaN(num) ? value : num.toFixed(precision);
    setMitutoyo(prev => ({ ...prev, value: formattedValue, status: isPass ? 'OK' : 'NG' }));
    if (isPass) {
      onComplete(detail.id, true);
    } else {
      onComplete(detail.id, false);
    }
  };
  const getStatusColorMitutoyo = (val, min, max) => {
    if (val === '' || val === null || isNaN(val)) return 'bg-yellow-400 border-gray-300 dark:border-gray-600';
  ...
  return (
    <article className="flex flex-col w-full rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden">
      ...
      <div className="flex items-center gap-4">
        <label className="w-20 font-bold text-center text-lg">{name}</label>
...
export default ProductCardMitutoyoMap;
